# Hierarchical Bayesian Atittudes Model Simulations 1 a & b

# The aim is to see how the higher and lower levels interact

# Intervention on the lower level (changing activations halfway, for n% of the epoch)

# Testing different profiles of gamma setups: i. lower level weighted more ii. top-down level weighted slightly more, as operative/functional metacognition iii. strong weighting on higher-level, lower level heavily discounted
...
params <- data.frame(A, I, Cp, tw, T, epsilon, lambda, gamma, gammaTAU, gammaE, tau_mean_T[1], mean(x), n_nodes, A_T2[1], mean_TAU1, mean_TAU2, R_T)
...
if(sim1a1){
p <- ggplot(mapping = aes(sumscores)) +
